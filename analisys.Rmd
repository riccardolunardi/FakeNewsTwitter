...
Il file prodotto dalla funzione _stream_tweets_ è un documento che raccoglie più JSON, uno per tweet raccolto. Avendo avuto solo qualche mese per raccogliere i dati, ogni giorno facevo ripartire lo streaming per evitare possibili bug o stalli dello script, per cui alla fine ho ottenuto un insieme di JSON, ognuno rinominato con il giorno in cui è stato creato.
...
I giorni che ho rimosso sono giorni in cui lo script si è bloccato, il server ha interrotto lo streaming oppure sono dati che sono inclusi nel file del giorno precedente.
...
Il costo computazionale di questo calcolo è molto alto: con il mio Intel i5-4590 la parte di codice sottostante impiega quasi 8 minuti per completarsi.
